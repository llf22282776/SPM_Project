...
public class EmailAction extends ActionSupport {
    ...
    public String emailNotify() throws Exception {
        ...
        System.out.println("****************emailFrom**************:"
                + emailFrom + "*********password*************:" + password);
        setEmailStyle();

        String msg = "";
        String studentIds = ServletActionContext.getRequest().getParameter(
                "studentIds[]");

        String[] stuIdArray = studentIds.split(",");
        StringBuffer failIdStrings = new StringBuffer();
        int count=0;
        for (int i = 0; i < stuIdArray.length; i++) {
            try {
                ...
                if (course.getFinalGrade().compareTo(new BigDecimal(60)) == -1) {
                    sendEmail(
                            "预警通知",
                            course.getName() + "同学，您的成绩为"
                                    + course.getFinalGrade()
                                    + "分，请留意课程平台的补考相关通知。", course.getEmail());
                    msg += "成功向</br>收件人: " + course.getName() + "("
                            + course.getStudentId() + ")发送邮件</br>";
                    count++;  
                } else {

                    sendEmail("成绩通知", "您的课程成绩为" + course.getFinalGrade()
                            + "分，详情请登录教务系统查询，请继续努力。", course.getEmail());
                    msg += "成功向</br>收件人: " + course.getName() + "("
                            + course.getStudentId() + ")发送邮件</br>";
                    count++;
                }
                ...
                e.printStackTrace();
                failIdStrings.append("未能向学号为"+stuIdArray[i]+"的同学发送邮件"+"<br />");
              
                
            }
        }
        if(count ==  stuIdArray.length){
            msg="发送邮件全部成功<br />"+msg;
            ...
            msg="发送邮件部分成功 <br />"+msg+failIdStrings.toString();
            Map request = (Map) ActionContext.getContext().get("request");
            request.put("msg", msg);
            return "error";
